...
Page-level skipping saves you the decompression and decoding cost of skipped pages, but it doesn't save you the overhead of entering the row group, opening column chunks, and scanning page headers to check their stats. For a very large row group with many pages, that scanning overhead adds up — especially on object storage where even metadata reads have latency.
